...
export function makeCourse(): Course {
  return { id: makeId(), code: "", title: "", creditUnit: 3, grade: "A" };
}
...
export function makeSemester(
  ...
  return { id: makeId(), level, semester, courses: [makeCourse()] };
}

export function courseQualityPoint(course: Course) {
  return (Number(course.creditUnit) || 0) * GRADE_POINTS[course.grade];
}

export function getStats(courses: Course[]) {
  const totalCU = courses.reduce((sum, c) => sum + (Number(c.creditUnit) || 0), 0);
  const totalQP = courses.reduce((sum, c) => sum + courseQualityPoint(c), 0);
  return { totalCU, totalQP, gp: totalCU ? totalQP / totalCU : 0 };
}

export function getCumulativeStats(records: Semester[]) {
  return getStats(records.flatMap((record) => record.courses));
}
...
export function semesterKey(record: Pick<Semester, "level" | "semester">) {
  return `${record.level}-${record.semester}`;
}

export function sortRecords(records: Semester[]) {
  return [...records].sort((a, b) => {
    const level = LEVELS.indexOf(a.level as (typeof LEVELS)[number]) -
...
export function validateRecords(records: Semester[]) {
  ...
  records.forEach((record) => {
    ...
    record.courses.forEach((course, index) => {
      ...
      if (!Number.isFinite(Number(course.creditUnit)) || Number(course.creditUnit) <= 0) {
        errors.push(`${record.level} ${record.semester}, course ${index + 1}: enter a valid credit unit`);
      }
    });
  });
