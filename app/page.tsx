...
export default function Home() {
  ...
  const selectedCourse =
    active?.courses.find((course) => course.id === selectedCourseId) ?? null;
  ...
  function addSemester(level: string, semester: SemesterName) {
    if (
      ...
      flash(
        "error",
        `${level} ${semester} is already in your academic record.`
      );
      return;
    }
  ...
  function reset() {
    if (
      !window.confirm(
        "Clear the entire academic record? This cannot be undone."
      )
    ) {
      return;
    }
          ...
          <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
            <div className="max-w-2xl">
              ...
              <p className="mt-2 text-sm leading-6 text-blue-100/80 sm:text-base">
                Add your courses semester by semester. Every credit unit and
                quality point counts toward one true cumulative result.
              </p>
            </div>
          ...
          <div className="flex items-center justify-between px-1 pb-3">
            <div>
              ...
              <p className="text-sm font-extrabold text-slate-900">
                {records.length} semester{records.length === 1 ? "" : "s"} added
              </p>
            </div>
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-xl bg-blue-600 px-3 py-2 text-xs font-bold text-white shadow-md shadow-blue-600/20 transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:bg-slate-300"
            >
              <Plus size={15} />
              Add semester
            </button>
              ...
              return (
                ...
                >
                  ...
                  <p className="mt-1 text-[11px] font-medium text-slate-500">
                    {record.semester} · {stats.totalCU} CU
                  </p>
                </button>
              );
            })}
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    Your courses stay compact. Tap any course code to view or
                    edit its details.
                  </p>
                </div>
            ...
            {active && (
              <div className="p-4 sm:p-6">
                <div className="mb-3 flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs text-slate-400">
                      {active.courses.length} course
                      {active.courses.length === 1 ? "" : "s"} ·{" "}
                ...
                <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-xs leading-5 text-slate-500">
                  <b className="text-slate-700">Tip:</b> Tap a course code to
                  edit its title, credit unit or grade.
                </div>
              </div>
            )}
          </section>
          ...
          <aside className="space-y-4">
            ...
            <div className="rounded-3xl border border-blue-100 bg-blue-50 p-5">
              ...
              <p className="mt-3 text-sm leading-6 text-blue-900/70">
                No averaging semester GPs. The calculator uses the actual
                credit-weighted quality points from every semester.
              </p>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Credit unit">
            <input
              ...
              onChange={(e) =>
                onUpdate(course.id, {
                  creditUnit: Number(e.target.value),
                })
              }
              className="modal-input"
            />
        ...
        <div className="flex items-center justify-between rounded-2xl bg-slate-50 p-4">
          ...
          <div className="text-right">
            <p className="text-xs text-slate-400">Grade point</p>
        ...
        <div className="flex gap-2 pt-1">
          ...
          <button
            onClick={onRemove}
            className="inline-flex items-center justify-center gap-2 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-extrabold text-red-600 hover:bg-red-100"
          >
            <Trash2 size={16} />
            Remove
          </button>
...
function SemesterModal({
  ...
  available: { level: string; semester: SemesterName }[];
  onClose: () => void;
  onAdd: (level: string, semester: SemesterName) => void;
}) {
  return (
    <ModalShell
      title="Add semester"
      subtitle="Each level can have one First Semester and one Second Semester."
      onClose={onClose}
    >
      {available.length === 0 ? (
        <EmptyState text="All 10 semesters have already been added." />
      ) : (
        <div className="grid gap-2 sm:grid-cols-2">
          {available.map((item) => (
            <button
              key={`${item.level}-${item.semester}`}
              onClick={() => onAdd(item.level, item.semester)}
              className="group flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 p-4 text-left transition hover:border-blue-200 hover:bg-blue-50"
            >
              <div>
...
function CourseModal({
  ...
  return (
    <ModalShell
      title="Add course"
      subtitle="Add one course at a time. The course list stays compact."
      onClose={onClose}
    >
      <div className="space-y-4">
        <Field label="Course code">
